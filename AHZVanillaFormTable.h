...
public:
	...
	typedef std::tr1::tuple<string, UInt32> AHZFormEntry;
   ...
   static void LoadACTIForms(CAHZFormLookup& loader)
   {
      ...
      ASSERT((AHZACTIForms.size() == AHZACTITargetForms.size()));
		...
		AHZLookupMap::const_iterator itEnd = AHZACTIForms.end();
		AHZLookupMap::const_iterator itSource = AHZACTIForms.begin();
		AHZLookupMap::const_iterator itTarget = AHZACTITargetForms.begin();
	...
	static void LoadVMVariables(CAHZFormLookup& loader)
	{
      ...
      const VMScriptEntries AHZVMVariables = CreateVMVariables();
		...
		VMScriptEntries::const_iterator itEnd = AHZVMVariables.end();
		VMScriptEntries::const_iterator it = AHZVMVariables.begin();
		while (it != itEnd) {
