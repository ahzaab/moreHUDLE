...
{	
	...
	if(pPC) 
	{
		...
		double fDamage = CALL_MEMBER_FN(pPC, GetDamage)(&objDesc);
...
const char * GetTargetName(TESObjectREFR *thisObject)
{
	...
	if(pPC) 
	{
		...
		PlayerCharacter::ObjDesc objDesc;
		objDesc.form = thisObject->baseForm;
		objDesc.extraData = new tList<BaseExtraList>();
		objDesc.extraData->Insert(&thisObject->extraData);
