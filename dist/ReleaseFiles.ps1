...
$sourceDir = "$($Env:SkyrimPath)"
$archiveToolDir = "$sourceDir"
$sourceDataDir = "$sourceDir\Data"
$releaseDir = "$($Env:ModDevPath)\MODS\SkyrimLE\moreHUD\Release"
...
$requiredDataDirs = @("Interface","Scripts","Scripts\Source" , "Interface\exported", "Interface\translations", "SKSE\Plugins")
...
$items += Get-ChildItem "$sourceDataDir\Scripts\Source" -Filter ahz*.psc
